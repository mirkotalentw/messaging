...
def data_extraction(prompt):
    completion = client.chat.completions.create(
                  model='gpt-4o',
                  temperature=0.9,
                  messages=[
                    {"role": "system", "content": prompt},
                    {"role": "user", "content": 'Generate me a template for a linkedin message to reach out to a candidate.'}
                ])
    ...
    if st.button('Generate Message'):
        if company_name and company_desc and job and person:
            ...
            with st.spinner('Generating text... Please wait'):
                gpt_output = data_extraction(formatted_prompt)
